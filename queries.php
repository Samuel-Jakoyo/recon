...
{
	 $query = "insert into cashbook (cdate,description,referenceno,debit,credit,accountno,branchcode)
               select unreconcb.cdate,unreconcb.description,unreconcb.referenceno,unreconcb.debit,unreconcb.credit,unreconcb.accountno,unreconcb.branchcode
                from unreconcb
                where status = '0'";        
	   $result = mysqli_query($db,$query);
	   if($result){
...
if(isset($_POST['update_user']))
{
	...
	$password_2 = mysqli_real_escape_string($db,$_POST['password_2']);
	$password = md5($password_1);
	mysqli_query($db, "update users set username='$username', password = '$password' where userid = '$id'");
	header("location:admin/create_user.php");
}
?>