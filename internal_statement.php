...
<head>
    ...
    <div class="panel">
        ...
        <div class="panel-body" id="maindiv">
            <a href="javascript:void(0);" onclick="$('#importFrm').slideToggle();" style="font-size:18px; text-decoration:none;">Import Internal Statement</a>
            
            <form action="importcashbook.php" method="post" enctype="multipart/form-data" id="importFrm">
...
<form method="post" action="functions.php">         
	   <div class="panel-body" id="maindiv">
            ...
            <table class="table" border="1px groove" style="border-color: #85929E">
                     <button type="submit" style="float:right"  class="button" name="clear_cash">Clear Records</button>
					 <button type="submit" style="float:right"  class="button" name="cash_btn">Move Records</button>
					 <button type="submit" style="float:right"  class="button" name="unreconcb">Bring Unreconciled</button>
                <thead>
                    <tr border="0.5px">
                        <th>Date</th>
                        <th>Description</th>
                ...
                <tbody>
                    <?php
                    //get rows query
                    $query = $db->query("SELECT * FROM cashbook");
                    if($query->num_rows > 0){ 
                        while($row = $query->fetch_assoc()){
                            if($row['status'] == 0)
                            {
							$_SESSION['cash']= $row["accountno"]; 
                        ?>
